...
def create_quiz_for_lesson(course_id, module_index, lesson_index, lesson_data):
    ...
    try:
        ...
        for q_data in questions:
            ...
            if q_id is None:
                q_id_row = db_select_one(conn, "SELECT LAST_INSERT_ID()", ())
                q_id = q_id_row[0] if q_id_row else None
                print(f"Debug: fallback LAST_INSERT_ID() returned {q_id}")

            if q_id is None:
                raise ValueError("Failed to resolve inserted question ID for quiz question")
...
def _generate_quiz_questions(lesson_title, lesson_summary, lesson_data):
    ...
    try:
        ...
        prompt = f"""
        Generate exactly 4 multiple choice quiz questions based on this lesson:
        ...
        - Mark exactly one correct answer with "is_correct": true
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are an expert educational assessment specialist creating clear, fair multiple choice quiz questions. Always generate exactly 4 questions with exactly 4 options each."
                },
        ...
        # Filter out invalid questions and ensure we have valid ones
        valid_questions = []
        for question in questions:
            if (question.get('question') and question['question'].strip() and 
                question.get('options') and len(question['options']) >= 2):
                # Ensure at least one correct answer
                has_correct = any(opt.get('is_correct', False) for opt in question['options'])
                if not has_correct:
                    # Set first option as correct if none marked
                    question['options'][0]['is_correct'] = True
                valid_questions.append(question)
        
        # If fewer than 4 valid questions, pad with fallbacks
        while len(valid_questions) < 4:
            fallback_question = {
            ...
            valid_questions.append(fallback_question)
        
        return valid_questions[:4]  # Ensure exactly 4
...
def get_lesson_quiz(course_id, module_index, lesson_index):
    ...
    try:
        ...
        # Get course, module, and lesson names
        course_info = db_select_one(conn, "SELECT title, content FROM courses WHERE id = %s", (course_id,))
        
        if course_info:
            course_name = course_info[0]
            course_data = json.loads(course_info[1]) if course_info[1] else {}
            modules = course_data.get('modules', [])
            
            if module_index < len(modules):
                module_data = modules[module_index]
                module_name = module_data.get('title', f'Module {module_index + 1}')
                lessons = module_data.get('lessons', [])
                
                if lesson_index < len(lessons):
                    lesson_data = lessons[lesson_index]
                    lesson_name = lesson_data.get('title', f'Lesson {lesson_index + 1}')
                else:
                    lesson_name = f'Lesson {lesson_index + 1}'
            else:
                module_name = f'Module {module_index + 1}'
                lesson_name = f'Lesson {lesson_index + 1}'
        else:
...
def create_saq_for_module(course_id, module_index, module_data):
    ...
    try:
        ...
        questions = _generate_saq_questions(module_title, module_summary, module_data)
...
def _generate_saq_questions(module_title, module_summary, module_data):
...
def get_module_saqs(course_id, module_index):
...
def submit_saq_response(user_id, course_id, module_index, saq_id, answer_text):
        """Submit a short answer question response and auto-grade using LLM."""
        conn = None
        try:
            conn = get_db_connection()
            ...
            db_insert(conn, """
                ...
                (user_id, course_id, module_index, saq_id, answer_text, is_graded, score, feedback, graded_by_user_id, graded_at)
                VALUES (%s, %s, %s, %s, %s, 0, NULL, NULL, NULL, NULL)
            ...
            question_data = db_select_one(conn, """
                SELECT question_text, rubric, max_score
                FROM short_answer_questions
                WHERE id = %s
            """, (saq_id,))

            score = 0
            feedback = 'Answer submitted successfully. LLM grading is in progress.'
            llm_grade = None

            if question_data:
                question_text, rubric, max_score = question_data
                rubric = json.loads(rubric) if rubric else {}

                llm_grade = llm_grade_saq_response(question_text, rubric or {}, answer_text, max_score)
                score = float(llm_grade.get('score', 0) or 0)
                feedback = llm_grade.get('feedback', feedback) or feedback
                ...
                db_update(conn, """
                    ...
                    SET score = %s,
                        ...
                        graded_by_user_id = NULL,
                        graded_at = NOW()
                    WHERE user_id = %s AND saq_id = %s
                """, (score, feedback, user_id, saq_id))

            return {
                'status': 'submitted',
                'message': 'Answer submitted and graded by LLM',
                'score': score,
                'feedback': feedback,
...
def grade_saq_response( response_id, score, feedback, graded_by_user_id):
...
def request_saq_reevaluation(user_id, response_ids, reason):
        """Create reevaluation requests for one or more SAQ responses."""
        conn = None
        try:
            conn = get_db_connection()
            ...
            for response_id in response_ids:
                ...
                if not existing:
                    raise ValueError('Invalid SAQ response for reevaluation')

                request_id = db_insert(conn, """
                    INSERT INTO saq_reevaluation_requests
                    (response_id, user_id, reason, status)
                    VALUES (%s, %s, %s, 'pending')
                """, (response_id, user_id, reason))
                request_ids.append(request_id)

            return {'status': 'pending', 'message': 'Reevaluation requested', 'request_ids': request_ids}

        except Exception as e:
            print(f"Error requesting reevaluation: {str(e)}")
            if conn:
                conn.rollback()
            raise
...
def create_final_assessment( course_id, course_data):
        """Create final assessment questions for the course"""
        try:
            conn = get_db_connection()
            
            # Create 3-5 essay questions for final assessment
            questions = _generate_final_questions(course_data)
...
def _generate_final_questions( course_data):
...
def get_final_assessments( course_id):
...
def submit_final_response(user_id, course_id, final_assessment_id, answer_text):
...
def grade_final_response(response_id, score, feedback, graded_by_user_id):
...
def calculate_module_assessment_score( user_id, course_id, module_index):
        """Calculate overall score for a module (lesson quizzes + SAQ average)"""
        try:
            conn = get_db_connection()
            
            # Get average quiz score across lessons in the module
            quiz_result = db_select_one(conn, """
                SELECT AVG(score_percentage) FROM lesson_quiz_results
                WHERE user_id = %s AND course_id = %s AND module_index = %s
            """, (user_id, course_id, module_index))
            
            quiz_avg = float(quiz_result[0]) if quiz_result and quiz_result[0] is not None else 0
            ...
            if saq_results:
                saq_scores = [Decimal(str(r[0])) for r in saq_results if r[0] is not None]
                saq_avg = float(sum(saq_scores) / len(saq_scores)) if saq_scores else 0
            else:
                saq_avg = 0
            
            # Combined module score (40% quizzes + 60% SAQ)
            module_score = quiz_avg * 0.40 + saq_avg * 0.60 if saq_results else quiz_avg
            ...
            if existing:
                db_update(conn, """
                    ...
                    WHERE user_id = %s AND course_id = %s AND module_index = %s
                """, (quiz_avg, saq_avg, module_score, user_id, course_id, module_index))
            else:
                db_insert(conn, """
                ...
                """, (user_id, course_id, module_index, quiz_avg, saq_avg, module_score, 1))
...
def calculate_final_grade( user_id, course_id, total_modules):
        """
        ...
        try:
            ...
            weighted_score = (module_avg * MODULE_ASSESSMENTS_WEIGHT) + \
                           (final_avg * FINAL_ASSESSMENT_WEIGHT)
            
            passed = 1 if weighted_score >= CERTIFICATE_THRESHOLD else 0
...
def llm_grade_saq_response( question_text, rubric, answer_text, max_score=10):
        """
...
def llm_grade_final_response( question_text, answer_text, key_concepts=None, max_score=20):
        """
...
def issue_certificate( user_id, course_id, user_name, course_title):
        ...
        try:
            ...
            cert_code = _generate_certificate_code(user_id, course_id)
